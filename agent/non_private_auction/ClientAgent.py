...
class ClientAgent(Agent):
    ...
    def send_orders(self, currentTime, total):
        num_real = 8
        price_buy = random.randint(99, 102)
        price_sell = random.randint(100, 103)
        if self.id <= total / 2:
            buy_sell = 'B'
            price = price_buy
        else:
        ...
        for order_id in range(0, num_real):
            order = (price, buy_sell, (self.id, client_name, True))
            orders.append(order)
        ...
        def send_execution_message(order,type):
            ...
            self.sendMessage(self.serviceAgentID,
                             Message({
                                 ...
                                 "order": order,
                                 "type" : type,
                                 "status": status  # Return the actual status of the order
                             }),
                             tag="comm_order_match")

        # Check first order
        if order1[1] == self.name:  # Check if the agent name matches
            send_execution_message(order1,"buy")
            self.matched_orders += 1
        elif order2[1] == self.name:  # Check if the agent name matches
